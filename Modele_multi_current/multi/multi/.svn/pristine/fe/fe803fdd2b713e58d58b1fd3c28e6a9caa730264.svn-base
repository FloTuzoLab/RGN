...
int main(int argc, char* argv[]) {
   ...
   /*Communicating the information of the .txt to the simulation about to run*/ filein>>line>>simulationNumber>>sizerConcentrationNum>>sizerCycleRegulation>>eta_trans>>cell_diff_ratio>>CG_diff_ratio>>desc_prop;
    ...
    double ratioBetweenGrowthandCooperationCost(1);
    double ratioBetweenK_offAndK_on(500);
    double allometryBasalAlpha(1.25);
    double minimumSynthesisDuration(1500);
    double ATPexpensesCalibration(pow(10,6));
    double volumeProportionDescendant(desc_prop);
    double volumeCellMitosis(40);
    int timeStepRatio(10);
    double etaTranslationActivityPostCheckpoint(eta_trans);
    double K_commonGoodsKinetics(pow(10,5));
    bool PGefficiencyMutation(false);
