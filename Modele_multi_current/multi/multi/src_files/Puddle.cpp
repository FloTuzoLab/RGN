...
Puddle::Puddle(int simulationNumber, bool cellSizerCycle, double delta_t, double delta_l, int timeStepRatio, int lengthPuddle,int widthPuddle, bool adjacencyNeed, Compartiment insideCompartment, Compartiment outsideCompartment, int initialNumberCell, Cell cell,ofstream* _fileoutcompartments,ofstream* _fileoutcells):m_lengthInside(lengthPuddle),m_widthInside(widthPuddle),m_adjacencyNeed(adjacencyNeed),m_cellSizerCycle(cellSizerCycle),fileoutcompartments(_fileoutcompartments),fileoutcells(_fileoutcells)
{
...
void Puddle::TimeStepInsidePuddle(int t_current, double delta_t){
    ...
    double totalMembraneSurface[m_lengthInside+1][m_widthInside+1]; //Total cumulated surface membrane of all the cells contained in a compartment
    double PublicGoodsReleased[m_lengthInside+1][m_widthInside+1]; //Public Goods released in any compartment
    double StandardNutrientsReleased[m_lengthInside+1][m_widthInside+1]; //Standard nutrients released in any compartment
    double RevealedNutrientsReleased[m_lengthInside+1][m_widthInside+1]; //Revealed nutrients released in any compartment
    ...
    }//Fixing these numbers to zero before counting
    ...
    for (int delta_t_current=0; delta_t_current< delta_t; delta_t_current++){
        ...
        for (int cell=0; cell<numberOfCellsBeforeTimestep;cell++){
            ...
            /* 2.Different outcomes of cell processing depending on the size of the cell*/
                ...
                if (m_cell[cell].MitosisCheckpoint()){  //
                    ...
                    bool energyUsefulness=m_cell[cell].EnergyBudgetUsefulnessTest(delta_t);
                    m_cell[cell].EnergyBalance(energyUsefulness,delta_t);
                    m_cell[cell].GarbageNeutralization(pow(10,5), delta_t);
        ...
        for (int l=1;l<=m_lengthInside;l++){
            for (int w=1;w<=m_widthInside;w++){
                ...
                m_puddle[l][w].HiddenNutrientBalance(delta_t);
            }
        }
    }
}
...
void Puddle::OutputGetting(int simulationNumber,int t_output)
    {
        for (int i=1;i<=m_lengthInside;i++)
        {
            for (int j=1;j<=m_widthInside;j++)
            {
                //cout<<"Type de compartiment du compartiment: ("<<i<<","<<j<<")"<<endl;
        (*fileoutcompartments)<<simulationNumber<<"\t"<<t_output<<"\t"<<m_puddle[i][j].GetConcentrationNutrientStandard()<<"\t"<<m_puddle[i][j].GetConcentrationNutrientHidden()<<"\t"<<m_puddle[i][j].GetConcentrationNutrientRevealed()<<"\t"<<m_puddle[i][j].GetConcentrationCommonGoods()<<"\t"<<m_cell.size()<<endl;
            }
        }
        //(*fileoutcompartments)<<endl;
        for (int c=1;c<=m_cell.size();c++)
        {
            /*
            ...
            string geneBarCode;
            m_cell[c-1].GeneTabularConversion(geneBarCode);
            (*fileoutcells)<<simulationNumber<<"\t"<<t_output<<"\t"<<m_cell[c-1].GetGenerationNumber()<<"\t"<<m_cell[c-1].GetMitosisThreshold()<<"\t"<<m_cell[c-1].GetVolumeCell()<<"\t"<<m_cell[c-1].GetMitosisSize()<<"\t"<<m_cell[c-1].GetProteins('s')<<"\t"<<m_cell[c-1].GetProteins('g')<<"\t"<<m_cell[c-1].GetProteins('m')<<"\t"<<m_cell[c-1].GetProteins('p')<<"\t"<<m_cell[c-1].printCellMatrix()<<"\t"<<geneBarCode << endl;
        }
    }
